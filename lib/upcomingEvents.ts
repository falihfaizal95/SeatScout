...
export interface HomepageEvent {
  id: string;
  title: string;
  date: string;       // "March 18, 2026 · 7:30 PM"
  location: string;
  imageUrl: string;
  sport: "NBA" | "NHL";
  tmUrl?: string;
...
function popularityScore(game: ESPNGame): number {
  ...
  // Primetime: 7–10 PM ET = roughly 23–03 UTC
...
async function enrichWithTicketmaster(game: ESPNGame, sport: "NBA" | "NHL"): Promise<TMResult> {
  if (!TM_KEY) return {};
...
export async function getUpcomingPopularEvents(count = 3): Promise<HomepageEvent[]> {
  ...
  const enriched = await Promise.all(
    top.map(async ({ game, sport }) => {
      ...
      const tm = await enrichWithTicketmaster(game, sport);
      ...
      return {
        ...
        title: game.name,
        date: formatEventDate(game.date),
